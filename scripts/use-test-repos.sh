...
# This script shows how to use the generated repos with repo-claude (rc)
...
show_usage_options() {
    ...
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo
    echo "The test framework already created an rc workspace for you."
    echo "Navigate to it and start using rc commands:"
    echo
    echo -e "${GREEN}cd $test_dir/rc-workspace/microservices-test${NC}"
    echo -e "${GREEN}$RC_BIN tree${NC}                  # View repository tree"
    echo -e "${GREEN}$RC_BIN use auth${NC}              # Navigate to auth service"
    echo -e "${GREEN}$RC_BIN clone --recursive${NC}     # Clone all lazy repos"
    echo -e "${GREEN}$RC_BIN status${NC}                # Check status"
    echo
    ...
    echo -e "$RC_BIN init my-project"
    echo -e "cd my-project"
    echo
    echo -e "${GREEN}# 2. Add repositories from test scenario${NC}"
    echo -e "$RC_BIN add file://$test_dir/auth-service --name auth"
    echo -e "$RC_BIN add file://$test_dir/user-service --name users"
    echo -e "$RC_BIN add file://$test_dir/payment-service --name payments --lazy"
    echo -e "$RC_BIN add file://$test_dir/common-lib --name common"
    echo
    echo -e "${GREEN}# 3. View and navigate${NC}"
    echo -e "$RC_BIN tree"
    echo -e "$RC_BIN use auth"
    echo
    ...
    if [ -f "$test_dir/test-config.yaml" ]; then
        ...
        echo -e "$RC_BIN init my-bulk-project"
        echo -e "cd my-bulk-project"
        echo
        echo -e "${GREEN}# 3. Manually add repos from config (rc doesn't support bulk import yet)${NC}"
...
demonstrate_usage() {
    ...
    echo -e "  ${GREEN}$RC_BIN init demo-project${NC}"
    if $RC_BIN init demo-project >/dev/null 2>&1; then
        echo -e "  ${GREEN}✓ Workspace initialized${NC}"
    else
        echo -e "  ${YELLOW}⚠ Init failed (rc may not be built)${NC}"
        return 1
    fi
    ...
    if [ -d "$test_dir/auth-service/.git" ]; then
        echo -e "  ${GREEN}$RC_BIN add file://$test_dir/auth-service --name auth${NC}"
        $RC_BIN add "file://$test_dir/auth-service" --name auth 2>/dev/null || true
    fi
    
    if [ -d "$test_dir/common-lib/.git" ]; then
        echo -e "  ${GREEN}$RC_BIN add file://$test_dir/common-lib --name common${NC}"
        $RC_BIN add "file://$test_dir/common-lib" --name common 2>/dev/null || true
    fi
    
    echo
    echo -e "${BLUE}Step 3: View the tree${NC}"
    echo -e "  ${GREEN}$RC_BIN tree${NC}"
    $RC_BIN tree 2>/dev/null || echo -e "  ${YELLOW}Tree command failed${NC}"
    
    echo
    echo -e "${BLUE}Step 4: Navigate to a repository${NC}"
    echo -e "  ${GREEN}$RC_BIN use auth${NC}"
    $RC_BIN use auth 2>/dev/null || echo -e "  ${YELLOW}Navigation failed${NC}"
...
main() {
    ...
    if [ ! -f "$RC_BIN" ]; then
        ...
        echo
        echo "However, here's how you WOULD use the test repos once rc is built:"
        echo
    fi
    ...
    if [ -d "$TEST_DIR/microservices" ]; then
        ...
        # Optionally run demo
        echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        echo -e "${CYAN}Want to see a live demo? (requires rc binary)${NC}"
        echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        echo
        echo -e "Run: ${GREEN}$0 $TEST_DIR/microservices --demo${NC}"
        
        if [ "${2:-}" = "--demo" ] && [ -f "$RC_BIN" ]; then
            echo
    ...
    echo "5. ${YELLOW}No recursion needed${NC}: rc handles the tree structure internally"
    echo
