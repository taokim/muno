...
func (m *StartModelV2) buildItems() []list.Item {
	...
	if len(m.config.Scopes) > 0 {
		...
		// Legacy mode: show agents as scopes
		for name, agent := range m.config.Agents {
			...
			for _, project := range m.config.Workspace.Manifest.Projects {
				if project.Agent == name {
					...
					repoToScope[project.Name] = name
				}
			}
			
			items = append(items, StartItemV2{
				Name:        name,
				Type:        "scope", // Treat agents as scopes for selection purposes
	...
	for _, project := range m.config.Workspace.Manifest.Projects {
		scopeContaining := repoToScope[project.Name]
		items = append(items, StartItemV2{
			Name:            project.Name,
			Type:            "repo",
