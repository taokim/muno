...
# Comprehensive testing for all MUNO features
...
clear
echo -e "${MAGENTA}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${MAGENTA}║           MUNO Regression Test Suite v2.0                    ║${NC}"
echo -e "${MAGENTA}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
for repo_spec in "${repos[@]}"; do
    ...
    if [[ "$repo_type" == "eager" ]]; then
        echo "eager=true" > .muno-meta
    elif [[ "$repo_type" == "config" ]]; then
        cat > muno.yaml << EOF
workspace:
  name: $repo_name
  repos_dir: nodes
...
test_case "List shows repositories" "$MUNO_BIN list" "backend-monorepo" || true
...
test_case "Verify lazy repo cloned" "test -d '$WORKSPACE_DIR/nodes/lib-common/.git'" "EXIST" || true
...
test_case "Added repo in list" "$MUNO_BIN list" "new-service" || true
test_case "Remove repository" "$MUNO_BIN remove new-service" "SUCCESS" || true
test_case "Removed repo not in list" "$MUNO_BIN list | grep -v new-service" "" || true
echo ""

# Category: Git Operations
echo -e "${MAGENTA}▶ Git Operations${NC}"
test_case "Status command works" "$MUNO_BIN status" "SUCCESS" || true
test_case "Status shows clean repos" "$MUNO_BIN status" "" || true

# Create changes for testing
if [[ -d "$WORKSPACE_DIR/nodes/normal-repo" ]]; then
    echo "test" > "$WORKSPACE_DIR/nodes/normal-repo/test.txt"
fi
test_case "Status detects changes" "$MUNO_BIN status normal-repo" "" || true
echo ""
...
test_case "Eager repo (platform) cloned" "test -d '$WORKSPACE_DIR/nodes/frontend-platform/.git'" "EXIST" || true
...
test_case "Config node has muno.yaml" "test -f '$WORKSPACE_DIR/nodes/team-config/muno.yaml'" "EXIST" || true
cd "$WORKSPACE_DIR"
echo ""
...
test_case "Remove non-existent repo" "$MUNO_BIN remove fake-repo 2>&1" "not found\|error" || true
echo ""
